...
public class ClsVehiculos {
     ...
     *                                       *
     ****************************************/
        public boolean GuardarDatosMemoriaRetirados(){
        
        boolean Guardado = false;
        
        Gson gson = new Gson();
        String jsonString = gson.toJson(ClsDB.jsonVehiculosRetirados);
        
        String[] arrSplitFile = this.toString().split("@"); 
        
        try{
            FileWriter fr = new FileWriter("vehiculosRetirados" + ".txt");
            fr.write(jsonString);
            fr.close();
            Guardado = true;
            ObtenerDatosMemoria();
        ...
        public void ObtenerDatosMemoriaRetirados(){        
        
            String[] arrSplitFile = this.toString().split("@"); 

            try {
                FileReader reader = new FileReader("vehiculosRetirados" + ".txt");
                BufferedReader bufferedReader = new BufferedReader(reader);
